...
assert.deepEqual({chars:Object.keys(D.CHARACTERS).length,styles:Object.keys(D.CHARACTER_STYLES).length,relics:Object.keys(D.RELICS).length,protocols:Object.keys(D.PROTOCOLS).length,tunings:Object.keys(D.TUNINGS).length,traits:Object.keys(D.TRAITS).length,behaviors:Object.keys(D.ENEMY_BEHAVIORS).length},{chars:38,styles:95,relics:385,protocols:155,tunings:62,traits:91,behaviors:108});
let s=BL.Store.mergeDefaults({version:23,boss4Defeated:true,alchemy:{enabled:true,allocation:{fire:6,earth:6},recipes:['lava'],hold:[]}});assert.equal(s.version,26);assert.equal(s.alchemy.allocation.fire,6);
